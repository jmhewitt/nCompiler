...
## Outputs 'Rhooks.R' for .onLoad() call.
writeRHooks <- function(pkgName, Rdir) {
## Serialization is the only client currently:  return if not serializing.

    deparsed_hooks = c(
    '## Generated by nCompiler::nWritePackage() -> do not edit by hand\n',
    '.onLoad <- function(...) {',
    paste0('## chargeLoadedFns(', pkgName, ')'),
    '}'
  )
...
chargeLoadedFns <- function(pkgName) {
  print(getDLLRegisteredRoutines(pkgName))
}
